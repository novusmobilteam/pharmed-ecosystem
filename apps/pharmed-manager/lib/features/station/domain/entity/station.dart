...
class Station extends Selectable implements TableData {
  ...
  StationDTO toDTO() {
    return StationDTO(
      id: id,
      name: name,
      stationDrug: drugStatus.isActive,
      stationMedicalConsumables: medicalConsumableStatus.isActive,
      service: service?.toDTO(),
      materialWarehouse: materialWarehouse?.toDTO(),
      medicalConsumableWarehouse: medicalConsumableWarehouse?.toDTO(),
      stationProvidedServices: services.map((s) => s.toDTO()).toList(),
    );
  }
}
