...
class FillingListLocalDataSource implements FillingListDataSource {
  ...
  FillingListLocalDataSource({required String recordsPath, required String detailsPath})
  ...
  Future<Result<List<FillingDetailDTO>>> getFillingListDetail(int fillingListId) async {
    ...
    return res.when(ok: (response) => Result.ok(response.data ?? []), error: Result.error);
  }
