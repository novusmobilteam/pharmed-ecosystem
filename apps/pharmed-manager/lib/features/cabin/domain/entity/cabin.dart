...
import '../../data/model/cabin_dto.dart';
...
class Cabin extends Selectable implements TableData {
  ...
  Cabin({
    ...
    this.parityBit,
    this.station,
  }) : super(title: name.toString(), subtitle: no?.toCustomString());
  ...
  CabinDTO toDTO() {
    return CabinDTO(
      ...
      type: type?.id,
      comPortsId: comPort?.id,
      baudRatesId: baudRate?.id,
      paritiesId: parityBit?.id,
      dataBits: dataBit?.id,
      stopBits: stopBit?.id,
      colorHexCode: color?.hex,
      sequenceNo: sequenceNo,
      //cardTypesIds: cardTypes.map((c) => c.id).toList(),
      cameraNo: cameraNo,
      dvrIp: dvrIp,
  ...
  @override
  List get content => [name, type?.label, no?.toCustomString()];
