...
class CreatePrescriptionWithProductsUseCase {
  ...
  Future<Result<List<PrescriptionItem>>> call({
    ...
    // 1) Reçete oluştur
    final rCreate = await _prescriptionRepository.createPrescription(prescription.copyWith(
    ...
    return rCreate.when(
      ...
      ok: (created) async {
        ...
        if (prescId == null) {
          return Result.error(
            CustomException(
              message: 'Reçete oluşturulurken bir hata oluştu. Lütfen daha sonra tekrar deneyiniz.',
            ),
          );
        }
