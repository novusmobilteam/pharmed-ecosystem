...
class WizardDraft extends Equatable {
  ...
  /// Mobil kabin — manuel tanım + port keşif sonucu
  final WizardMobileLayout? mobileLayout;

  /// Mobil kabin — port keşfinde yanıt veren port numaraları.
  ///
  /// Port keşfi tamamlanana kadar boş listedir.
  /// Keşif sonrası [mobileLayout.applyDiscoveredPorts] ile
  /// çekmecelere uygulanır.
  ///
  /// Örn: [1, 2, 3, 4] → port 1-4 aktif, port 5-8 yanıt vermedi.
  ...
  /// Mobil kabin için port keşfi yapıldı mı?
  /// Keşif yapılmamışsa step4 devam butonu pasif kalır.
  bool get portDiscoveryComplete => cabinType != CabinType.mobile || discoveredPorts.isNotEmpty;

  bool get isComplete => step1Complete && step2Complete && step3Complete && step4Complete && portDiscoveryComplete;
  ...
  @override
  List<Object?> get props => [cabinType, basicInfo, serviceScope, scannedLayout, mobileLayout, discoveredPorts];
}
