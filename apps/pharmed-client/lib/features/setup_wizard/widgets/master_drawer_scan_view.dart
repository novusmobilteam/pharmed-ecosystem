...
// lib/features/setup_wizard/presentation/widgets/master_drawer_scan_view.dart
//
// [SWREQ-SETUP-UI-007] [IEC 62304 §5.5]
// Master kabin wizard adım 4 — cihaz tarama arayüzü.
// Sınıf: Class B

class MasterDrawerScanView extends ConsumerWidget {
  const MasterDrawerScanView({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(step4MasterNotifierProvider);
    final notifier = ref.read(step4MasterNotifierProvider.notifier);

    return switch (state.scanState) {
      DrawerScanState.idle => _ScanIdle(onScan: notifier.scanDevice),
      DrawerScanState.scanning => _ScanInProgress(logs: state.scanLogs),
      DrawerScanState.found => _ScanFound(
        logs: state.scanLogs,
        scannedLayout: state.scannedLayout,
        onReset: notifier.resetScan,
      ),
      DrawerScanState.error => _ScanError(logs: state.scanLogs, onRetry: notifier.scanDevice),
    };
  }
}
...
class _ScanInProgress extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final headerMessage = logs.isNotEmpty
        ? (logs.last.headerMessage ?? context.l10n.wizard_scanningStatus)
        : context.l10n.wizard_scanningStatus;
        ...
        children: [
          ...
          if (compartmentCount > 0)
            Container(
              ...
              child: Text(
                isKubik
                    ? context.l10n.wizard_cellCountLabel(compartmentCount)
                    : context.l10n.wizard_rowCountLabel(group.units.length),
                style: TextStyle(
                  fontFamily: MedFonts.mono,
                  fontSize: 11,
                  fontWeight: FontWeight.w600,
