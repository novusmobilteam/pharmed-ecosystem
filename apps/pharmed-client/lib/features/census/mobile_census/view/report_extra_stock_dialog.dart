...
class ReportExtraStockDialog extends ConsumerStatefulWidget {
  const ReportExtraStockDialog({super.key});

  static Future<({Medicine medicine, double quantity})?> show(BuildContext context) {
    return showDialog<({Medicine medicine, double quantity})>(
      context: context,
      builder: (_) => const ReportExtraStockDialog(),
    );
  }

  @override
  ConsumerState<ReportExtraStockDialog> createState() => _ReportExtraStockDialogState();
}

class _ReportExtraStockDialogState extends ConsumerState<ReportExtraStockDialog> {
  Medicine? _selected;
  double _quantity = 1;
      ...
      child: Column(
        ...
        children: [
          MedSelectionField<Medicine>(
            ...
            dataSource: (skip, take, query) => ref
                .read(getMedicinesUseCaseProvider)
                .call(PagedQueryParams(skip: skip, take: take, searchQuery: query)),
