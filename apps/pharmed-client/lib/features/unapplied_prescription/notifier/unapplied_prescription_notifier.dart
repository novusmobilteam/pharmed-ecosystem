...
final unappliedPrescriptionNotifierProvider = ChangeNotifierProvider<UnappliedPrescriptionNotifier>((ref) {
  return UnappliedPrescriptionNotifier(
...
class UnappliedPrescriptionNotifier extends ChangeNotifier with ApiRequestMixin {
  ...
  UnappliedPrescriptionNotifier({required GetPatientPrescriptionHistoryUseCase getPatientPrescriptionHistoryUseCase})
  ...
  Future<void> _fetch() async {
    ...
    await execute(
      ...
      onData: (data) {
        _items = data.where((item) => item.status == PrescriptionMovementType.purchasePending).toList();
        notifyListeners();
      },
    );
  }
