...
class MasterUnloadNotifier extends ChangeNotifier with ApiRequestMixin, CabinDrawerQueueMixin<CabinOperationTarget> {
  MasterUnloadNotifier({
    required MasterDrawerOrchestrator orchestrator,
    required GetCabinAssignmentsUseCase getAssignments,
  ...
  final GetCabinAssignmentsUseCase _getAssignments;
  ...
  /// Kullanıcının boşaltma için işaretlediği fiziksel göz (unit) id'lerinin kümesi.
  ...
  Future<void> startUnload() async {
    ...
    final targets = selectedAssignments.map((a) => CabinOperationTarget.fromAssignment(a, unloadTargetConfig)).toList();
    final result = CabinDrawerQueueBuilder.build<CabinOperationTarget>(items: targets);

    if (result.jobs.isEmpty) {
      reportError(const CabinValidationFailure(reason: CabinValidationReason.noDrawerFound));
      return;
    }
  ...
  Future<Result<void>> submitTarget(CabinOperationTarget target) {
    ...
    if (params.isEmpty) return Future.value(const Result.ok(null)); // boş göz — confirmCurrent zaten eliyor
    return executeAsResult(submitTargetOp, operation: () => _completeUnload.call(params));
  }

  // Kübik girdileri
  void onCubicCountChanged(double? v) => updateCurrentTarget((t) => t.withCubicCount(v ?? 0));
  void onCubicUnloadChanged(double? v) => updateCurrentTarget((t) => t.withCubicSecondary(v ?? 0));
  void onCubicMiadChanged(DateTime? d) => updateCurrentTarget((t) => t.withCubicMiad(d));

  // Birim doz girdileri
  void onStepCountChanged(int index, double? v) => updateCurrentTarget((t) => t.withStepCount(index, v ?? 0));
  void onStepUnloadChanged(int index, double? v) => updateCurrentTarget((t) => t.withStepSecondary(index, v ?? 0));
