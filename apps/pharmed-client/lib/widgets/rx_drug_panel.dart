...
import 'hospitalization_detail_banner.dart';
...
class _RxDrugPanelState extends State<RxDrugPanel> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        HospitalizationDetailBanner(hospitalization: widget.hospitalization),
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: MedSpacing.xl4, vertical: MedSpacing.xl2),
          child: _RxPanelHeader(
            title: widget.title,
            items: widget.showFilters ? widget.items : const [],
            activeFilter: _activeFilter,
            onFilterChanged: (status) => setState(() => _activeFilter = status),
          ),
        ),
