...
class CabinCellInputCard extends StatelessWidget {
  const CabinCellInputCard({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final hasEntry = _showFilling ? (fillingQuantity ?? 0) > 0 : (countQuantity ?? 0) > 0;
    final isExpired = miadDate.isExpiredMiad;
    final miadHasError = showMiad && ((hasEntry && miadDate == null) || isExpired);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _inputs(context, showMiad, miadText, miadHasError, unitSuffix),
        ],
      ),
    );
  }
  ...
  Widget _inputs(BuildContext context, bool showMiad, String miadText, bool miadHasError, String? unitSuffix) {
    ...
    final fillingCard = _showFilling
        ? MedValueCard(
            density: density,
    ...
    if (!showMiad) {
      return Row(
        ...
        children: [
          ...
          if (fillingCard != null) Expanded(child: fillingCard),
        ],
      );
    }
    ...
    // Alan sayısına göre flex oranı: 2 alan (sayım, sadece count+miad) eşit
    // paylaşım; 3 alan (dolum, count+filling+miad) 2/2/3.
    if (!_showFilling) {
      return Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        spacing: 8,
