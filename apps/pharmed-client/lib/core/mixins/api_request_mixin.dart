...
mixin ApiRequestMixin on ChangeNotifier {
  ...
  @override
  void dispose() {
    _disposed = true; // YENİ — her şeyden önce işaretle
    super.dispose();
  }
  ...
  /// executeVoid'i Result<void> dönüşüne çeviren yardımcı — özellikle
  /// CabinDrawerQueueMixin.submitTarget gibi Result bekleyen dış
  /// sözleşmelere ApiRequestMixin'in loading/log altyapısını bağlamak için.
  Future<Result<void>> executeAsResult(
    OperationKey key, {
    required Future<Result<void>> Function() operation,
    String? loadingMessage,
    String? successMessage,
    ...
    await executeVoid(
      key,
      operation: operation,
      onFailed: (e) => result = Result.error(e),
