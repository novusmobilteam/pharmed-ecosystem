          ...
          child: SingleChildScrollView(
            ...
            child: cabinetType == CabinetType.standard
                ? _StandardDrawerPanel(
                    config: drawerConfig is StandardDrawerConfig
                        ? drawerConfig as StandardDrawerConfig
                        : null,
                    scanState: scanState,
                    onScan: onScanDevice,
                ...
                : _MobileDrawerPanel(
                    config: drawerConfig is MobileDrawerConfig
                        ? drawerConfig as MobileDrawerConfig
                        : null,
                    onChanged: onConfigChanged,
                  ),
          ),
...
class _StandardDrawerPanelState extends State<_StandardDrawerPanel> {
  ...
  void _emitManual() {
    widget.onChanged(StandardDrawerConfig(
      sections: _sections,
      drawerType: _drawerType,
      depth: _depth,
      scannedFromDevice: false,
    ));
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        if (widget.scanState == DrawerScanState.found &&
            widget.config != null) ...[
...
class _ScanInProgressState extends State<_ScanInProgress>
      ...
      children: [
        Container(
          ...
          child: const Icon(Icons.check_circle_rounded,
              size: 28, color: Color(0xFF0D9E6C)),
        ),
        const SizedBox(height: 12),
        const Text(
          'Cihaz Bulundu',
      ...
      child: Row(
        children: [
          const Icon(Icons.inventory_2_outlined,
              size: 20, color: Color(0xFF0D9E6C)),
            ...
            children: [
              ...
              Expanded(
                ...
                child: MedSelectField<DrawerType>(
                  label: 'Çekmece Tipi',
                  value: drawerType,
                  options: DrawerType.values
                      .map((t) => MedSelectOption(label: t.label, value: t))
                      .toList(),
...
class _MobileDrawerPanelState extends State<_MobileDrawerPanel> {
  ...
  List<MobileDrawerRow> _buildDefaultRows(int count) {
    return List.generate(
      ...
      (i) => MobileDrawerRow(
        rowIndex: i + 1,
        drawerType: DrawerType.cubic4x4,
        columns: 2,
      ),
    );
  }
  ...
  void _emit() {
    widget.onChanged(MobileDrawerConfig(
      drawerCount: _rowCount,
      rows: _rows,
    ));
  }
    ...
    return Column(
      ...
      children: [
        ...
        for (var i = 0; i < _rows.length; i++) ...[
          _MobileRowCard(
            row: _rows[i],
            onTypeChanged: (t) => _onRowTypeChanged(i, t),
          ),
          const SizedBox(height: 8),
        ],
      ],
