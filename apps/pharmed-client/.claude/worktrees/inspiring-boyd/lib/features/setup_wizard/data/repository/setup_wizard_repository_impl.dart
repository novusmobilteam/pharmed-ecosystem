...
class SetupWizardRepositoryImpl {
  ...
  Future<Result<StandardDrawerConfig>> scanDeviceDrawerConfig(
      String ipAddress) async {
    final result = await dataSource.scanDeviceDrawerConfig(ipAddress);
    return result.fold(
      (dto) => Success(_toStandardDrawerConfig(dto)),
      Failure.new,
    );
  }
  ...
  CabinConfigDto _toDto(CabinSetupConfig config) {
    ...
    return CabinConfigDto(
      ...
      sslEnabled: config.basicInfo.sslEnabled,
