...
class Step5Summary extends StatelessWidget {
    ...
    return Column(
      children: [
        StepHeader(
          ...
          title: 'Kurulum Özeti',
          subtitle:
              'Tüm ayarları gözden geçirin. Onayladıktan sonra kabin sisteme kaydedilir.',
        ),
  ...
  List<_SummaryRow> _buildBasicInfoRows(WizardDraft draft) {
    ...
    return [
      ...
      _SummaryRow(label: 'SSL', value: info.sslEnabled ? 'Etkin' : 'Devre Dışı'),
    ];
  }
  ...
  List<_SummaryRow> _buildDrawerRows(WizardDraft draft) {
    ...
    if (config is StandardDrawerConfig) {
      return [
        ...
        if (config.drawerType == DrawerType.unitDose)
          _SummaryRow(label: 'Derinlik', value: '${config.depth}'),
        _SummaryRow(
            label: 'Kaynak',
            value:
                config.scannedFromDevice ? 'Cihazdan Okundu' : 'Manuel Girildi'),
      ];
      ...
      return [
        ...
        for (final row in config.rows)
          _SummaryRow(
            label: 'Sıra ${row.rowIndex}',
            value: row.drawerType.label,
          ),
      ];
    }
    return [];
  ...
  List<_SummaryRow> _buildScopeRows(WizardDraft draft) {
    final scope = draft.serviceScope;
    if (scope == null) return [];
    if (scope is ServiceBased) {
      return [
        _SummaryRow(label: 'Kapsam', value: 'Servis Bazlı'),
        _SummaryRow(label: 'Servis', value: scope.serviceName),
      ];
    } else if (scope is RoomBased) {
      return [
        _SummaryRow(label: 'Kapsam', value: 'Oda Bazlı'),
        _SummaryRow(
            label: 'Odalar',
            value: scope.rooms.isEmpty ? '—' : scope.rooms.join(', ')),
        _SummaryRow(label: 'Oda Sayısı', value: '${scope.rooms.length}'),
      ];
    }
    return [];
  }
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              border: const Border(
                  bottom: BorderSide(color: MedColors.border2)),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            decoration: const BoxDecoration(
              border:
                  Border(bottom: BorderSide(color: MedColors.border2)),
              color: MedColors.surface2,
              borderRadius:
                    ...
                    Expanded(
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(2),
                          border:
                              Border.all(color: const Color(0xFFC8D2E0)),
                        ),
                      ),
                    ),
