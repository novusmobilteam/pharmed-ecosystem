...
class MedSegmentedControl<T> extends StatelessWidget {
  const MedSegmentedControl({super.key, required this.options, required this.value, required this.onChanged});
    ...
    return Container(
      ...
      child: Row(
        ...
        children: options.map((opt) {
          ...
          return _SegmentButton(label: opt.label, isActive: isActive, onTap: () => onChanged(opt.value));
        }).toList(),
      ),
    );
  }
