...
class MedDateField extends StatefulWidget {
  const MedDateField({super.key, this.value, this.onChanged, this.label, this.helperText, this.onCalendarTap});
...
class _MedDateFieldState extends State<MedDateField> {
  ...
  void initState() {
    ...
    _dayCtrl = TextEditingController(text: v != null ? v.day.toString().padLeft(2, '0') : '');
    _monCtrl = TextEditingController(text: v != null ? v.month.toString().padLeft(2, '0') : '');
    _yrCtrl = TextEditingController(text: v != null ? v.year.toString() : '');
  ...
  void _onFocusChange() {
    final anyFocused = _dayFocus.hasFocus || _monFocus.hasFocus || _yrFocus.hasFocus;
  ...
  void _notifyChange() {
    ...
    if (day != null && mon != null && yr != null && _yrCtrl.text.length == 4) {
      try {
        ...
        AnimatedContainer(
          ...
          child: Row(
            children: [
              ...
              if (widget.onCalendarTap != null) _CalendarButton(onTap: widget.onCalendarTap!),
            ],
          ),
        ),
...
class _DateSep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
      '/',
      style: const TextStyle(fontFamily: MedFonts.mono, fontSize: 16, color: MedColors.text3),
    );
  }
}
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: const Border(left: BorderSide(color: MedColors.border2)),
        ),
        child: Icon(Icons.calendar_month_outlined, size: 16, color: _hovered ? MedColors.blue : MedColors.text3),
      ),
    );
  }
}
