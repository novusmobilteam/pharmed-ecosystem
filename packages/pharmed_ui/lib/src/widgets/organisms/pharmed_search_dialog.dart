...
class PharmedSearchDialog<T extends Selectable> extends StatefulWidget {
  const PharmedSearchDialog({
    super.key,
    required this.title,
  ...
  static Future<T?> show<T extends Selectable>(
    ...
    return showDialog<T>(
      ...
      builder: (_) => PharmedSearchDialog<T>(
        title: title,
  ...
  @override
  State<PharmedSearchDialog<T>> createState() => _PharmedSearchDialogState<T>();
}

class _PharmedSearchDialogState<T extends Selectable> extends State<PharmedSearchDialog<T>> {
  // ── State ────────────────────────────────────────────────────────
  final List<T> _items = [];
  int _totalCount = 0;
  ...
  Future<void> _fetch({bool reset = false}) async {
    ...
    // Daha fazla kayıt yoksa çekme
    if (!reset && _items.length >= _totalCount && _totalCount > 0) return;
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            _DialogHeader(title: widget.title, onClose: () => Navigator.of(context).pop()),
            _SearchBar(controller: _searchController, onChanged: _onSearchChanged),
            Flexible(child: _buildList()),
            _DialogFooter(
              selectedLabel: _selected != null ? widget.labelBuilder(_selected!) : null,
              onConfirm: _selected != null ? _onConfirm : null,
            ),
          ],
        ),
      ),
...
class _SearchListItem extends StatelessWidget {
  const _SearchListItem({required this.label, required this.isSelected, required this.onTap, this.subtitle});
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children: [
            ...
            if (isSelected) Icon(Icons.check_circle_rounded, size: 16, color: MedColors.blue),
          ],
        ),
      ),
    );
